...
async function logActivity(action: string, details: string) {
    const user = await getCurrentUser();
    const logs = await readData<ActivityLog>('logs.json');
    ...
    await writeData('logs.json', logs.slice(0, 1000)); // Keep last 1000 logs
}
...
async function logStockMovement(
    ...
    const user = await getCurrentUser();
    const movements = await readData<StockMovement>('stockMovements.json');
...
export async function addUnit(unit: Omit<Unit, 'id'>) {
    ...
    units.push(newUnit);
    await writeData('units.json', units);
...
export async function updateUnit(unitId: string, unitData: Partial<Unit>) {
    let units = await readData<Unit>('units.json');
...
export async function updatePatient(patientId: string, patientData: Partial<Patient>) {
    let patients = await readData<Patient>('patients.json');
...
export async function addDispensation(dispensationData: Omit<Dispensation, 'id' | 'date'>): Promise<Dispensation> {
    ...
    await writeData('dispensations.json', dispensations);
